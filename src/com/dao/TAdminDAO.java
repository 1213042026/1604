...
public class TAdminDAO extends HibernateDaoSupport {
	...
	public List findAll(String username,String realname) {
		log.debug("finding all TAdmin instances");
		try {
			String queryString = "from TAdmin as model where 1=1 and model.roles<>'管理员' ";
			if(username!=null&&!username.equals("")){
				queryString+=" and model.username like '%"+username+"%'";
			}
			if(realname!=null&&!realname.equals("")){
				queryString+=" and model.realname like '%"+realname+"%'";
			}
			return getHibernateTemplate().find(queryString);
		} catch (RuntimeException re) {
			log.error("find all failed", re);
